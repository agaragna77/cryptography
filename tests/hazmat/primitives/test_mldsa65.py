...
def test_mldsa65_unsupported(backend):
    ...
    with raises_unsupported_algorithm(
        ...
        MlDsa65PrivateKey.from_seed_bytes(b"0" * 2560)
...
class TestMlDsa65Signing:
    def test_sign_verify(self, backend):
        ...
        # ML-DSA-65 signatures are 3309 bytes according
    ...
    def test_invalid_length_from_public_bytes(self, backend):
        with pytest.raises(ValueError):
            MlDsa65PublicKey.from_public_bytes(b"a" * 1311)
        with pytest.raises(ValueError):
            MlDsa65PublicKey.from_public_bytes(b"a" * 1313)

    def test_invalid_length_from_private_bytes(self, backend):
        with pytest.raises(ValueError):
            MlDsa65PrivateKey.from_seed_bytes(b"a" * 2559)
        with pytest.raises(ValueError):
            MlDsa65PrivateKey.from_seed_bytes(b"a" * 2561)
...
def test_rfc9881_seed_only_format(backend):
    """
    RFC 9881 Section 6 defines ML-DSA-65-PrivateKey as a CHOICE with three
    formats:
