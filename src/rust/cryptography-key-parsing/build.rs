    ...
    if env::var("DEP_OPENSSL_BORINGSSL").is_ok() {
        ...
        if let Ok(openssl_dir) = env::var("OPENSSL_DIR") {
            ...
            if include_dir.is_dir() {
                let probe_path = Path::new(env!("CARGO_MANIFEST_DIR")).parent().map(|p| p.join("probe_mldsa.c"));
        ...
        if !mldsa_supported {
            ...
            if let Ok(ver) = env::var("CRYPTOGRAPHY_BORINGSSL_VERSION") {
                if ver.split('.').nth(1).and_then(|s| s.parse::<u32>().ok()).is_some_and(|d| d >= BORINGSSL_MLDSA_MIN_DATE) {
